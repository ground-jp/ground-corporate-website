...
サイトの編集方法は大きく分けて2つあります。用途に応じて使い分けてください。

| 方法 | 対象者 | 必要なもの | できること |
|------|--------|-----------|-----------|
| GDE（ブラウザ編集） | 全スタッフ | ブラウザのみ | テキスト・画像・色の変更 |
| コード編集（Git） | 開発担当者 | Git + テキストエディタ | レイアウト変更・新ページ追加・全ての変更 |

### 7.1 GDE（ビジュアルデザインエディタ）

#### 概要

GDE（GROUND Design Editor）は、HTMLの知識がなくてもブラウザ上でサイトのコンテンツを編集できるツールです。PCにソフトをインストールする必要はなく、ブラウザからURLにアクセスするだけで使えます。

#### アクセスURL

- **GDE**: https://super-pastelito-06a52d.netlify.app/admin/design-editor.html
- **DNS切り替え後**: https://groundinc.co.jp/admin/design-editor.html

※ どのPCからでもブラウザで上記URLを開くだけでアクセスできます。Git や VS Code のインストールは不要です。
...
#### Netlify Forms ダッシュボード（推奨）

新しいお問い合わせは Netlify Forms に届きます。

1. https://app.netlify.com/ にログイン
2. サイトを選択
3. 「Forms」タブ → 「contact」フォームを選択
4. 送信されたお問い合わせ一覧が確認できます

#### 旧管理画面（レガシー）

- **URL**: https://super-pastelito-06a52d.netlify.app/admin/leads.html
- **DNS切り替え後**: https://groundinc.co.jp/admin/leads.html
- ※ こちらは旧システム（Supabase）のデータのみ表示されます。新規のお問い合わせは Netlify Forms ダッシュボードで確認してください。

### 7.3 コード編集（Git を使った編集）

HTMLやCSSを直接編集する場合は、ローカルPCにリポジトリが必要です。

#### ローカルフォルダを削除してしまった場合

コードはすべて GitHub に保存されているため、いつでも復元できます：

```bash
cd ~/Desktop
git clone https://github.com/ground-jp/ground-corporate-website.git website
cd website
```

これで元通りに編集・デプロイができるようになります。
